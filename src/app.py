...
def main():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["🏆 Match Prediction", "📊 Team Analysis", "📈 Statistics", "📁 Data Upload"]
    )
...
def render_match_prediction():
    ...
    if st.button("🔮 Predict Match Outcome", type="primary"):
        ...
        score_text = (
            f"**{home_team} {predicted_score['home_goals']} - "
            f"{predicted_score['away_goals']} {away_team}**"
        )
        st.write(score_text)
...
def render_team_analysis():
    ...
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(x=matches, y=goals_scored, name="Goals Scored", line=dict(color="green"))
    )
    fig.add_trace(
        go.Scatter(x=matches, y=goals_conceded, name="Goals Conceded", line=dict(color="red"))
    )
...
def render_statistics():
    ...
    with col1:
        ...
        fig = px.histogram(x=goals_data, title="Goals per Match Distribution", nbins=20)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col2:
        ...
        fig.update_layout(title="Home vs Away Goals", xaxis_title="Goals", yaxis_title="Frequency")
        st.plotly_chart(fig, use_container_width=True)
...
def render_data_upload():
    ...
    st.download_button(
        "📥 Download Sample Data", data=csv, file_name="sample_football_data.csv", mime="text/csv"
    )
